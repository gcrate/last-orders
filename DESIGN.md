...
- **Balance departures from placeholders** (from the harness, default policy): regrowth is 5–8% per generation, not 10–15%, because at 10–15% later keepers did worse than earlier ones; a rest day costs 0.85 days of life instead of 1 ("slightly"), since at 0.5 a keeper who always rested lived twice as long. With these numbers the default policy wins about 64% of campaigns, in 2.7 generations on average (about 8% in the first generation, roughly 25% per later one), losing about 9 adventurers per generation.
- **Art pipeline departures (ASSETS.md):** keeper and heir illness stages 2 and 3 are Pixen edits (`edit_image_pixen`) of stage 1, and stage 4 is an edit of stage 3, so the illness never looks milder at a later stage (edits of stage 1 alone came out too subtle or out of order). `ui_button` is generated at 48×32 and cropped to 48×16, because Pixen only takes square canvases below 32px; the crop keeps the pixel grid. Item icon prompts that came back as characters were rewritten ("object only, no person", "laid flat"); the manifest holds the prompt actually used.
